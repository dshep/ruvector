//! Configuration system for Ruvector-Mathpix
//!
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            model: ModelConfig {
                model_path: "models/mathpix-ocr".to_string(),
            ...
            cache: CacheConfig {
                ...
                persistent: false,
                cache_dir: ".cache/mathpix".to_string(),
            },
        }
    }
}
...
impl Config {
    ...
    ///
    /// ```rust,no_run
    /// use ruvector_mathpix::Config;
    ///
    /// let config = Config::from_file("mathpix.toml").unwrap();
    /// ```
    ...
    fn apply_env_overrides(&mut self) -> Result<()> {
        ...
        if let Ok(val) = std::env::var("MATHPIX_OCR__CONFIDENCE_THRESHOLD") {
            self.ocr.confidence_threshold = val.parse()
                .map_err(|_| MathpixError::Config("Invalid confidence_threshold".to_string()))?;
        }
        if let Ok(val) = std::env::var("MATHPIX_OCR__TIMEOUT") {
            self.ocr.timeout = val.parse()
                .map_err(|_| MathpixError::Config("Invalid timeout".to_string()))?;
        }
        if let Ok(val) = std::env::var("MATHPIX_OCR__USE_GPU") {
            self.ocr.use_gpu = val.parse()
                .map_err(|_| MathpixError::Config("Invalid use_gpu".to_string()))?;
        }
        ...
        if let Ok(val) = std::env::var("MATHPIX_MODEL__BATCH_SIZE") {
            self.model.batch_size = val.parse()
                .map_err(|_| MathpixError::Config("Invalid batch_size".to_string()))?;
        }

        // Cache overrides
        if let Ok(val) = std::env::var("MATHPIX_CACHE__ENABLED") {
            self.cache.enabled = val.parse()
                .map_err(|_| MathpixError::Config("Invalid cache enabled".to_string()))?;
        }
        if let Ok(val) = std::env::var("MATHPIX_CACHE__CAPACITY") {
            self.cache.capacity = val.parse()
                .map_err(|_| MathpixError::Config("Invalid cache capacity".to_string()))?;
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.ocr.confidence_threshold < 0.0 || self.ocr.confidence_threshold > 1.0 {
            return Err(MathpixError::Config(

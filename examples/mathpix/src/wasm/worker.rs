...
use crate::wasm::api::MathpixWasm;
use crate::wasm::types::RecognitionFormat;

static WORKER_INSTANCE: OnceCell<Arc<MathpixWasm>> = OnceCell::new();
...
pub async fn init_worker() -> Result<(), JsValue> {
    let instance = MathpixWasm::new().await?;
...
async fn process_image(id: String, image_data: Vec<u8>, format: String) -> Result<(), JsValue> {
    ...
    let mut worker_instance = MathpixWasm::new().await?;

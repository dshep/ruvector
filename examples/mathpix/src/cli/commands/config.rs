...
pub enum ConfigCommand {
    ...
    Init {
        ...
        #[arg(short, long, default_value = "mathpix.toml")]
        output: PathBuf,
    ...
    Show {
        /// Path to config file (default: from --config or mathpix.toml)
        #[arg(value_name = "FILE")]
        file: Option<PathBuf>,
    },
...
fn init_config(output: &PathBuf, force: bool) -> Result<()> {
    ...
    println!("  mathpix-cli --config {} <command>", output.display());
...
fn show_config(file: Option<PathBuf>) -> Result<()> {
    let config_path = file.unwrap_or_else(|| {
        PathBuf::from("mathpix.toml")
    });

    if !config_path.exists() {
        println!("No configuration file found.");
        println!("\nCreate one with:");
        println!("  mathpix-cli config init");
        return Ok(());
    }
...
fn show_config_path() -> Result<()> {
    if let Some(config_dir) = dirs::config_dir() {
        let app_config = config_dir.join("mathpix");
    ...
    println!("  mathpix-cli --config /path/to/config.toml <command>");
    println!("\nOr set environment variable:");

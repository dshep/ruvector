...
RELEASE_TAG="mathpix-models-v1.0.0"

# Model configurations
declare -A MODELS=(
    ["mathpix_encoder.onnx"]="https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/mathpix_encoder.onnx"
    ["mathpix_decoder.onnx"]="https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/mathpix_decoder.onnx"
    ["mathpix_tokenizer.onnx"]="https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/mathpix_tokenizer.onnx"
)

# SHA256 checksums (these should be updated with actual checksums)
declare -A CHECKSUMS=(
    ["mathpix_encoder.onnx"]="SHA256_PLACEHOLDER"
    ["mathpix_decoder.onnx"]="SHA256_PLACEHOLDER"
    ["mathpix_tokenizer.onnx"]="SHA256_PLACEHOLDER"
)
...
for model in "${!MODELS[@]}"; do
    ...
    if download_file "${MODELS[$model]}" "${output_path}"; then
        ...
        HF_URL="https://huggingface.co/ruvnet/mathpix-models/resolve/main/${model}"
...
2. Hugging Face: https://huggingface.co/ruvnet/mathpix-models
...
cat > "${MODELS_DIR}/config.json" << EOF
{
  "models": {
    "encoder": {
      "path": "mathpix_encoder.onnx",
      "type": "image_encoder",
      "input_shape": [1, 3, 224, 224],
      "output_dim": 768
    },
    "decoder": {
      "path": "mathpix_decoder.onnx",
      "type": "sequence_decoder",
      "vocab_size": 50000,
      "max_length": 512
    },
    "tokenizer": {
      "path": "mathpix_tokenizer.onnx",
      "type": "tokenizer",
      "vocab_size": 50000
    }
  },

// Configuration tests for ruvector-mathpix
//
...
mod config_tests {
    ...
    impl Default for OcrModelConfig {
        fn default() -> Self {
            Self {
                model_path: PathBuf::from("models/mathpix_model.onnx"),
    ...
    impl Default for MathpixConfig {
        fn default() -> Self {
            Self {
    ...
    fn test_default_config_creation() {
        let config = MathpixConfig::default();
    ...
    fn test_model_config_defaults() {
        ...
        assert_eq!(config.model_path, PathBuf::from("models/mathpix_model.onnx"));
    ...
    fn test_toml_deserialization() {
        let toml_str = r#"
            ...
            [model]
            model_path = "models/mathpix_model.onnx"
            device = "cpu"
            batch_size = 4
            confidence_threshold = 0.7
        ...
        let config: MathpixConfig = toml::from_str(toml_str).expect("Failed to deserialize TOML");
    ...
    fn test_preset_configurations() {
        // High quality preset
        let high_quality = MathpixConfig {
            preprocessing: PreprocessConfig {
                target_dpi: 600,
    ...
    fn test_config_cloning() {
        let config1 = MathpixConfig::default();
    ...
    fn test_config_roundtrip_toml() {
        let original = MathpixConfig::default();
        let toml_str = toml::to_string(&original).unwrap();
        let deserialized: MathpixConfig = toml::from_str(&toml_str).unwrap();
    ...
    fn test_config_roundtrip_json() {
        ...
        let deserialized: MathpixConfig = serde_json::from_str(&json_str).unwrap();

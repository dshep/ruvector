# Mathpix API Server Implementation

## Overview

A production-ready REST API server implementing the Mathpix v3 API specification using Axum framework. The server provides OCR, mathematical equation recognition, and async PDF processing capabilities.
...
### 1. Complete Mathpix v3 API Endpoints
...
# Production
cargo build --release --bin mathpix-server
...
RUST_LOG=mathpix_server=debug,tower_http=debug
...
# Check logs
RUST_LOG=debug cargo run --bin mathpix-server
```

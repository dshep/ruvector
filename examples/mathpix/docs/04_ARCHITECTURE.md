...
Ruvector-mathpix is a high-performance Rust-based OCR system specialized for mathematical content extraction, inspired by Mathpix. The architecture leverages ruvector's vector database capabilities for intelligent caching, semantic search of mathematical structures, and integration with agentic workflows through lean-agentic orchestration.
...
└── examples/
    └── mathpix/                           # Main mathpix directory
        ...
        │   ├── ruvector-mathpix-models/   # Model management
        │   ├── ruvector-mathpix-api/      # REST API server
        │   ├── ruvector-mathpix-cli/      # Command-line interface
        │   └── ruvector-mathpix-wasm/     # WebAssembly bindings
...
```
                ...
                │                             │
    ┌───────────▼───────────┐    ┌───────────▼────────────┐
    │ ruvector-mathpix-     │    │  ruvector-mathpix-     │
    │ models                │    │  core                  │
    ...
    └───────────┬───────────┘    └───────────┬────────────┘
          ...
          │                    │                    │
┌─────────▼──────────┐  ┌──────▼────────┐  ┌───────▼────────────┐
│ ruvector-mathpix-  │  │ ruvector-     │  │ ruvector-mathpix-  │
│ api                │  │ mathpix-cli   │  │ wasm               │
...
#### 2.3.2 ruvector-mathpix-models

**Purpose**: ML model management, loading, and caching

```toml
[package]
name = "ruvector-mathpix-models"
version = "0.1.0"
edition = "2021"

[dependencies]
ruvector-core = { path = "../../../crates/ruvector-core" }
ruvector-mathpix-core = { path = "../ruvector-mathpix-core" }
...
#### 2.3.3 ruvector-mathpix-api

**Purpose**: REST API server with WebSocket support

```toml
[package]
name = "ruvector-mathpix-api"
version = "0.1.0"
edition = "2021"
...
ruvector-core = { path = "../../../crates/ruvector-core" }
ruvector-mathpix-core = { path = "../ruvector-mathpix-core" }
ruvector-mathpix-models = { path = "../ruvector-mathpix-models" }
...
#### 2.3.4 ruvector-mathpix-cli

**Purpose**: Command-line interface tool

```toml
[package]
name = "ruvector-mathpix-cli"
version = "0.1.0"
edition = "2021"
...
[dependencies]
ruvector-mathpix-core = { path = "../ruvector-mathpix-core", default-features = false }
ruvector-mathpix-models = { path = "../ruvector-mathpix-models" }
...
impl CpuRuntime {
    pub fn new() -> Self {
        let thread_pool = rayon::ThreadPoolBuilder::new()
            ...
            .thread_name(|idx| format!("mathpix-rayon-{}", idx))
            .build()
            .unwrap();
...
impl ErrorRecovery {
    ...
    fn is_retryable(&self, error: &MathpixError) -> bool {
        matches!(error,
            MathpixError::Timeout(_) |
            MathpixError::ApiError { status: 503, .. } |
            MathpixError::InferenceError(_)
        )
    }
}
```
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<MathpixConfig> {
        ...
        if self.config_path.extension() == Some("toml".as_ref()) {
            ...
            Err(MathpixError::InvalidConfig("Unsupported format".into()))
        }
    }

    /// Load with environment variable overrides
    pub fn load_with_env(&self) -> Result<MathpixConfig> {
    ...
    fn apply_env_overrides(&self, config: &mut MathpixConfig) -> Result<()> {
...
RUN cargo build --release --bin ruvector-mathpix-api
...
COPY --from=builder /app/target/release/ruvector-mathpix-api /usr/local/bin/

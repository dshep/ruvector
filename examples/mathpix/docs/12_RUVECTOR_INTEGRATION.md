...
## ruvector-mathpix Integration Design
...
This document defines the integration architecture for `ruvector-mathpix`, a specialized OCR crate for mathematical expressions, with the existing ruvector ecosystem. The integration leverages ruvector's high-performance vector database, HNSW indexing, distributed clustering, and WASM capabilities to provide scalable, intelligent mathematical OCR processing.
...
members = [
    ...
    "crates/ruvector-mathpix-core",      # Core OCR logic
    "crates/ruvector-mathpix-node",      # Node.js bindings
    "crates/ruvector-mathpix-wasm",      # Browser WASM
    "crates/ruvector-mathpix-server",    # HTTP server extension

    "examples/refrag-pipeline",
    "examples/mathpix",                   # Examples and demos
]

[workspace.dependencies]
# ... existing dependencies ...

# Mathpix-specific dependencies - NEW
...
- Add mathpix-specific deps to workspace.dependencies for consistency

### 1.2 Crate Structure

```
crates/
├── ruvector-mathpix-core/      # Core OCR engine
│   ├── src/
│   │   ├── lib.rs              # Public API
│   │   ├── api_client.rs       # Mathpix API client
...
│   └── Cargo.toml
│
├── ruvector-mathpix-node/      # Node.js bindings (NAPI-RS)
│   ├── src/
│   │   └── lib.rs
│   ├── npm/                    # Platform binaries
│   └── Cargo.toml
│
├── ruvector-mathpix-wasm/      # WASM bindings
│   ├── src/
│   │   └── lib.rs
│   └── Cargo.toml
│
└── ruvector-mathpix-server/    # Server extension
    ├── src/
    │   ├── main.rs
    │   ├── routes.rs
    │   └── middleware.rs
...
examples/mathpix/               # Examples (NOT workspace member)
├── src/
├── tests/
├── docs/
...
**Core Crate (ruvector-mathpix-core):**
...
**crates/ruvector-mathpix-server/Cargo.toml:**

```toml
[package]
name = "ruvector-mathpix-server"
version.workspace = true
edition.workspace = true
license.workspace = true
authors.workspace = true
repository.workspace = true
description = "HTTP server for Mathpix OCR with vector caching"

[dependencies]
# Core dependencies
ruvector-core = { version = "0.1.16", path = "../ruvector-core" }
ruvector-server = { version = "0.1.16", path = "../ruvector-server" }
ruvector-mathpix-core = { version = "0.1.16", path = "../ruvector-mathpix-core" }
...
[features]
default = ["api-client"]
api-client = ["ruvector-mathpix-core/api-client"]
metrics = ["ruvector-metrics"]
```

### 3.2 REST API Endpoints

**crates/ruvector-mathpix-server/src/routes.rs:**
...
use ruvector_mathpix_core::{MathpixClient, MathpixCache};
use std::sync::Arc;

#[derive(Clone)]
pub struct AppState {
    pub mathpix_client: Arc<MathpixClient>,
    pub cache: Arc<parking_lot::RwLock<MathpixCache>>,
}

/// Create Mathpix routes
pub fn mathpix_routes() -> Router<AppState> {
    Router::new()
        // Mathpix API v3 endpoints
...
async fn ocr_text(
    ...
    let embedding = state.mathpix_client
    ...
    // Cache miss - call Mathpix API
    let result = state.mathpix_client.ocr_image(&image_data).await?;
...
async fn ocr_pdf(
    ...
    let pages = state.mathpix_client.extract_pdf_pages(&pdf_data)?;
    let results = futures::future::join_all(
        pages.into_iter().map(|page| {
            let client = state.mathpix_client.clone();
...
**crates/ruvector-mathpix-wasm/Cargo.toml:**
...
ruvector-mathpix-core = {
    version = "0.1.16",
    path = "../ruvector-mathpix-core",
    default-features = false,
    features = ["wasm"]
}
...
**crates/ruvector-mathpix-wasm/src/lib.rs:**
...
impl MathpixWasm {
    ...
    pub fn new(api_key: String, app_id: String) -> Result<MathpixWasm, JsValue> {
        console_error_panic_hook::set_once();

        let client = MathpixClient::new(api_key, app_id)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        // Use in-memory cache for WASM
        let cache = MathpixCache::new_memory(512) // 512-dim embeddings
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
...
**crates/ruvector-mathpix-wasm/mathpix.d.ts:**
...
**crates/ruvector-mathpix-core/src/metrics.rs:**
...
```rust
// In MathpixClient implementation
impl MathpixClient {
...
**crates/ruvector-mathpix-core/src/distributed.rs:**
...
# Cache Configuration
MATHPIX_CACHE_DIR=./data/mathpix_cache
...
url = "https://api.mathpix.com/v3"
timeout_secs = 30

[cache]
enabled = true
dir = "./data/mathpix_cache"
dimension = 512
size_mb = 1000
threshold = 0.95
...
**crates/ruvector-mathpix-core/src/config.rs:**
...
impl MathpixConfig {
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        ...
        let config: MathpixConfig = toml::from_str(&expanded)?;
        Ok(config)
    }
        ...
        Ok(Self {
            api: ApiConfig {
                ...
                url: std::env::var("MATHPIX_API_URL")
                    .unwrap_or_else(|_| "https://api.mathpix.com/v3".to_string()),
                timeout_secs: 30,
            },
...
**crates/ruvector-mathpix-core/src/error.rs:**
...
**crates/ruvector-mathpix-core/src/traits.rs:**
...
**crates/ruvector-mathpix-core/src/types.rs:**
...
members = [
    ...
    # Mathpix Integration
    "crates/ruvector-mathpix-core",
    "crates/ruvector-mathpix-node",
    "crates/ruvector-mathpix-wasm",
    "crates/ruvector-mathpix-server",
]
...
**crates/ruvector-mathpix-core/Cargo.toml:**

```toml
[package]
name = "ruvector-mathpix-core"
version.workspace = true
edition.workspace = true
license.workspace = true
authors.workspace = true
...
```
crates/ruvector-mathpix-core/src/
├── lib.rs                      # Public API
├── error.rs                    # Error types
...
```
crates/ruvector-mathpix-server/src/
...
- [ ] Create `ruvector-mathpix-core` crate
- [ ] Implement vector cache using `ruvector-core`
- [ ] Add Mathpix API client
- [ ] Implement image preprocessing
- [ ] Add metrics collection
- [ ] Write unit tests

### Phase 2: Server Extension
- [ ] Create `ruvector-mathpix-server` crate
...
- [ ] Create `ruvector-mathpix-wasm` crate
...
- [ ] Create `ruvector-mathpix-node` crate
...
### Key Metrics
- `mathpix_ocr_requests_total` - Total requests
- `mathpix_cache_hit_rate` - Cache effectiveness
- `mathpix_ocr_duration_seconds` - Latency distribution
- `mathpix_confidence_score` - Quality tracking
- `mathpix_errors_total` - Error rate
...
```bash
cd crates/ruvector-mathpix-core
...
let cache = MathpixCache::new("./cache", 512)?;
```

**Step 3:** Integrate metrics
```rust
use ruvector_mathpix_core::metrics::OcrMetrics;
...
This integration architecture provides a comprehensive blueprint for incorporating `ruvector-mathpix` into the ruvector ecosystem. By leveraging existing infrastructure for vector storage, clustering, metrics, and WASM support, we achieve:
...
2. Create Phase 1 crates (`ruvector-mathpix-core`)

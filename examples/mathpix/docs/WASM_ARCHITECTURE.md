...
The Mathpix WASM module provides browser-based OCR with LaTeX support through a carefully designed architecture optimizing for performance and developer experience.
...
Provides the main `MathpixWasm` class with methods:
...
impl MathpixWasm {
    ...
    pub async fn new() -> Result<MathpixWasm, JsValue> { ... }
}
```
...
impl From<MathpixError> for JsValue {
    fn from(error: MathpixError) -> Self {
...
```javascript
import { createMathpix } from './index.js';

test('recognizes text', async () => {
    const mathpix = await createMathpix();
    const result = await mathpix.recognize(testImage);
